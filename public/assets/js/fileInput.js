...
(function (factory) {
  ...
  defaultFileTypeSettings = {
    ...
    audio: function (vType, vName) {
      return (
        compare(vType, 'audio.*') &&
        (compare(vType, /(ogg|mp3|mp?g|wav)$/i) || compare(vName, /\.(og?|mp3|mp?g|wav)$/i))
      );
    },
  ...
  isEmpty = function (value, trim) {
    return (
      value === undefined || value === null || value.length === 0 || (trim && $.trim(value) === '')
    );
  };
  isArray = function (a) {
  ...
  FileInput.prototype = {
    ...
    _init: function (options) {
      ...
      self.slug =
        typeof options.slugCallback === 'function' ? options.slugCallback : self._slugDefault;
    ...
    _parseFileType: function (file) {
      ...
      for (i = 0; i < defaultPreviewTypes.length; i += 1) {
        ...
        isValid = isSet(cat, self.fileTypeSettings)
    ...
    _validateDefaultPreview: function () {
      ...
      self.$preview.html(
        '<div class="file-default-preview">' + self.defaultPreviewContent + '</div>',
      );
      self.$container.removeClass('file-input-new');
    },
    ...
    _getLayoutTemplate: function (t) {
      var self = this,
        template = isSet(t, self.layoutTemplates)
          ? self.layoutTemplates[t]
    ...
    _getPreviewTemplate: function (t) {
      var self = this,
        template = isSet(t, self.previewTemplates)
          ? self.previewTemplates[t]
    ...
    _previewFile: function (i, file, theFile, previewId, data) {
      ...
      var self = this,
        ...
        id,
        config = isSet(cat, self.previewSettings)
          ? self.previewSettings[cat]
          : defaultPreviewSettings[cat],
        chkMimes = mimes && mimes.indexOf(file.type) !== -1,
        footer = self._renderFileFooter(caption, config.width),
        modal = '',
        ind = previewId.slice(previewId.lastIndexOf('-') + 1);
        ...
        if (cat === 'text') {
          ...
          content =
            content
              ...
              .replace(/\{zoomInd}/g, self.zoomIndicator)
              .replace(/\{footer}/g, footer)
              .replace(/\{fileindex}/g, ind)
              .replace(/\{type}/g, file.type)
              .replace(/\{zoomTitle}/g, self.msgZoomTitle)
    ...
    _renderMain: function () {
      var self = this,
        dropCss =
          self.isUploadable && self.dropZoneEnabled ? ' file-drop-zone' : 'file-drop-disabled',
      ...
      return self.mainTemplate
        .replace(/\{class}/g, self.mainClass)
    ...
    _renderButton: function (type) {
      ...
      css +=
        type === 'browse' ? ' btn-file' : ' fileinput-' + type + ' fileinput-' + type + '-button';
      if (!isEmpty(label)) {
    ...
    _renderFileFooter: function (caption, width) {
      ...
      if (self.isUploadable) {
        footer = template.replace(
          /\{actions}/g,
          self._renderFileActions(true, true, false, false, false),
        );
        out = footer
          .replace(/\{caption}/g, caption)
          .replace(/\{width}/g, width)
    ...
    disable: function () {
      ...
      self.$container
        .find('.btn-file, .fileinput-remove, .fileinput-upload, .file-preview-frame button')
        .attr('disabled', true);
      self._initDragDrop();
      return self.$element;
    },
